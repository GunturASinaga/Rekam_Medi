...
<body>
    ...
    <div class="container">
        ...
        <div class="form-floating">
            <form action="/editPenyakitProcess" method="POST">
                @csrf
                ...
                <div class="form-floating my-3">
                    <input type="date" class="form-control" id="tanggal_mulai" name="tanggal_mulai"
                        value="{{ $penyakit['tanggal_mulai'] }}" required>
                    <label for="tanggal_mulai">Tanggal Mulai</label>
                </div>


                <div class="form-floating my-3">
                    <textarea class="form-control" id="tindakan_medis" name="tindakan_medis" rows="4" required>{{ $penyakit['tindakan_medis'] }}</textarea>
                ...
                <div class="form-floating my-3">
                    <input type="text" class="form-control" id="hasil" name="hasil"
                        value="{{ $penyakit['hasil'] }}" required>
                    <label for="hasil">Hasil</label>
                </div>


                <div class="form-check">
                    <input type="checkbox" class="form-check-input" id="sudah_selesai" name="sudah_selesai"
                        value="1" {{ $penyakit['tanggal_selesai'] ? 'checked' : '' }}>
                    <label class="form-check-label" for="sudah_selesai">Sudah Selesai?</label>
                </div>

                <div id="tanggal_selesai_container" {{ $penyakit['tanggal_selesai'] ? '' : 'style=display:none' }}>

                    <div class="form-floating my-3">
                        <input type="date" class="form-control" id="tanggal_selesai" name="tanggal_selesai"
                            value="{{ $penyakit['tanggal_selesai'] }}">
    ...
    <script>
        ...
        if (sudahSelesaiCheckbox.checked) {
